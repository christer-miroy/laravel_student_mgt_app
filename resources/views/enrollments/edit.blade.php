  ...
  <div class="card-body">
    <form action="{{ url('enrollments/' . $enrollments->id) }}" method="post">
      ...
      <select name="batch_id" id="batch_id" class="form-control mt-2">
          <option value="select_batch" disabled>Select Batch</option>
          @foreach($batches as $id => $name)
              <option value="{{ $id }}" {{ $id == $enrollments->batch_id ? 'selected' : '' }}>{{ $name }}</option>
          @endforeach
      </select>
      <label class="mt-2">Student Name</label>
      <select name="student_id" id="student_id" class="form-control mt-2">
          <option value="select_student" disabled>Select Student</option>
          @foreach($students as $id => $name)
              <option value="{{ $id }}" {{ $id == $enrollments->student_id ? 'selected' : '' }}>{{ $name }}</option>
          @endforeach
      </select>
